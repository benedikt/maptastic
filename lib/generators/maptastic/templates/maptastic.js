...
Maptastic.Map = Class.create({
	...
	setupCenter: function() {
		with(this) {
			if(element.readAttribute("data-map-center") == "client") {
				map.setCenter(new google.maps.LatLng(google.loader.ClientLocation.latitude, google.loader.ClientLocation.longitude))
			} else if(element.readAttribute("data-map-center") == "marker") {
				// Todo: Center map on geographic center of all markers
			} else if(element.hasAttribute("data-map-center")) {
				map.setCenter(Maptastic.toLatLng(element.readAttribute("data-map-center")));
			}
		}
	},
